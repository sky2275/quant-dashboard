...
def generate_html(market, us_market, flow_data, limit_up, transmissions, pos_rows, total_pnl, sector_stocks):
    ...
    for t in transmissions[:6]:
        ...
        stock_list = ""
        for s in stocks[:4]:
            stock_color = "#81c784" if s['signal'] in ['🟢持有', '🟢关注'] else "#e57373" if s['signal'] == '🔴观望' else "#ffd54f"
            stock_list += f'''
...
<head>
    ...
    <style>
        ...
        * {{ margin:0; padding:0; box-sizing:border-box; }}
        ...
        .stock-item {{ display:flex; justify-content:space-between; align-items:center; padding:3px 0; border-bottom:1px solid rgba(255,255,255,0.03); }}
...
def main():
    ...
    print(f"✅ 看板 v4 修复版已生成: index.html")
